...
export function BreadcrumbNav({ items, className }: BreadcrumbNavProps) {
  return (
    <nav aria-label="Breadcrumb" className={cn('border-b border-secondary-100 bg-secondary-50', className)}>
      <Container>
        <ol className="flex items-center gap-1 py-1 text-sm flex-wrap">
          {items.map((item, i) => (
            <li key={item.label} className="flex items-center gap-1">
              ...
              {item.href && i < items.length - 1 ? (
                <Link
                  href={item.href}
                  className="inline-flex items-center min-h-[44px] px-1.5 text-secondary-600 hover:text-primary-700 hover:underline underline-offset-4"
                >
                  {item.label}
                </Link>
              ) : (
