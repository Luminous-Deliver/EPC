...
import { pricing, nextDayGuide, expressFee, formatPrice, type PropertyType } from '@/lib/site'

/**
 * Floor-area guide estimator.
 *
 * The whole point of this component is the information hierarchy: floor area
 * is the real pricing driver, so m² is the dominant element in every band and
 * the bedroom description is a muted caption underneath. Reversing those two
 * is what previously led customers to believe bedroom count *was* the price
 * rule, and that the resulting number was final.
 *
 * A banded selector is used rather than a slider (which would imply a
 * precision guide pricing doesn't have) or a six-column table (which cannot
 * survive 390px without hiding data).
 */
  ...
  return (
    <div className="mt-8">
      ...
      <p className="mt-2 max-w-2xl text-secondary-700">
        Internal floor area (m²) is the main factor in our pricing. Bedroom count is shown underneath
        only as a familiar reference — pick whichever you know.
      </p>

      <div className="mt-5 grid gap-5 lg:grid-cols-12">
        {/* Bands — m² leads, bedrooms caption */}
        ...
        {/* Result — every figure labelled as a guide */}
        <div className="lg:col-span-5">
          <div className="rounded-2xl border border-secondary-200 bg-white p-5 shadow-premium">
            ...
            <p className="mt-1 text-2xl font-bold tracking-tight text-secondary-900">
              {band.areaLabel}
            </p>
            ...
            <dl className="mt-4 divide-y divide-secondary-100 border-t border-secondary-100">
              ...
              <div className="flex items-baseline justify-between gap-3 bg-accent-50/60 py-2.5">
                <dt className="text-sm font-semibold text-secondary-900">
                  Both together
                  <span className="block text-xs font-normal text-secondary-600">
                    Better value in one visit
                  </span>
                </dt>
